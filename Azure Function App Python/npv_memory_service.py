...
# Mirror npv_archive_service patterns: lazy SDK import, per-slot table name,
# thread-safe client cache, schemaless rows, soft-delete via Status.

_TABLE_CLIENT_CACHE: dict = {}
_CACHE_LOCK = threading.Lock()

TABLE_NAME_PREFIX = "NonprofitVerificationMemory"
SCHEMA_VERSION = 1
...
_SCOPE_KEY_RE = re.compile(r"^[A-Za-z0-9_\-]{1,32}$")
...
def _get_table_client(slot_name: str, logger=logging):
    ...
    try:
        ...
        if not connection_string:
            logger.error("npv_memory_service - AzureWebJobsStorage connection string not found")
            return None
        ...
        logger.info(f"npv_memory_service - Created table client for: {table_name}")
        return table_client
    except Exception as e:
        logger.error(f"npv_memory_service - Failed to create table client for {table_name}: {e}")
        return None
    ...
    for k, v in list(entity.items()):
        try:
            if isinstance(v, str) and len(v) > _MAX_STR_PROP_CHARS:
                ...
                logger.warning(
                    f"npv_memory_service - Truncated property '{k}' from {original_len} to {len(entity[k])} chars"
                )
        except Exception:
            pass
...
def _normalize_scope_key(scope_key: str) -> str:
    ...
    if not _SCOPE_KEY_RE.match(s):
        raise ValueError(f"scope_key '{scope_key}' must match [A-Za-z0-9_-]{{1,32}}")
    # ISO country codes are uppercased; 'global' stays lowercase.
...
# ---------------------------------------------------------------------------
# Content validation (defends against prompt-injection-style poisoning)
# ---------------------------------------------------------------------------
_FORBIDDEN_URL_PATTERNS = [
    re.compile(r"^(?!https?://)", re.IGNORECASE),  # must start http(s)
    re.compile(r"@", re.IGNORECASE),                # no userinfo
    re.compile(r"://(localhost|127\.|0\.0\.0\.0|10\.|192\.168\.|169\.254\.|172\.(1[6-9]|2\d|3[01])\.)", re.IGNORECASE),
    re.compile(r"://\[?[0-9a-f:]+\]?(/|$)", re.IGNORECASE),  # raw IPv4/IPv6 literal
]
...
def _validate_url(url: str) -> Optional[str]:
    ...
    for pat in _FORBIDDEN_URL_PATTERNS:
        if pat.search(url):
            return f"url rejected by safety pattern"
    return None
...
def _validate_content(category: str, content: Any) -> Optional[str]:
    ...
    url_fields = ("url", "url_template", "lookup_url", "host_url")
    for f in url_fields:
        v = content.get(f)
        if v is None:
            continue
        if "{" in str(v) and "}" in str(v):
            # URL template: validate after stripping {placeholders} for the host check
...
def record_entry(
    ...
    try:
        if category not in ALLOWED_CATEGORIES:
            logger.warning(f"npv_memory_service - record_entry: category '{category}' not allowed")
            return None
        if source not in ALLOWED_SOURCES:
            source = "agent"
        content_err = _validate_content(category, content)
        if content_err:
            logger.warning(f"npv_memory_service - record_entry: content rejected: {content_err}")
            return None
        ...
        if existing is not None:
            # Update existing row: bump success, refresh content if provided,
            # never downgrade Source from manual.
            now = _utcnow()
            ...
            if (entity.get("Status") or "active") == "deprecated":
                # Don't auto-revive deprecated rows; require manual reactivation.
                entity["ReviewNotes"] = (
        ...
        logger.error(f"npv_memory_service - record_entry error: {e}")
        return None
...
def _upsert_with_truncation(table_client, entity: dict, logger=logging) -> None:
    ...
    try:
        ...
        logger.error(f"npv_memory_service - upsert failed, retrying with reduced payload: {ex}")
...
def record_feedback(
    ...
    try:
        if outcome not in {"success", "failure"}:
            logger.warning(f"npv_memory_service - record_feedback: invalid outcome '{outcome}'")
            return None
        pk, rk = parse_ref(ref)
        slot = slot_name or get_slot_name()
        table_client = _get_table_client(slot, logger)
        ...
        # ETag-based optimistic concurrency, max 3 attempts.
        from azure.data.tables import UpdateMode
        ...
        for attempt in range(3):
            ...
            if entity is None:
                logger.warning(f"npv_memory_service - record_feedback: ref not found: {ref}")
                return None
            entity = dict(entity)
            now = _utcnow()
            if outcome == "success":
            ...
            # If failures dominate, flip to needsReview (unless manual-pinned).
            try:
                s = int(entity.get("SuccessCount", 0) or 0)
                f = int(entity.get("FailureCount", 0) or 0)
            ...
            try:
                ...
                if attempt == 2:
                    logger.warning(f"npv_memory_service - record_feedback: concurrency conflict after 3 attempts for {ref}")
                    return None
                continue
        return None
    except Exception as e:
        logger.error(f"npv_memory_service - record_feedback error: {e}")
        return None
...
def bump_use_count(refs: Iterable[str], slot_name: Optional[str] = None, logger=logging) -> int:
    ...
    try:
        ...
        logger.error(f"npv_memory_service - bump_use_count error: {e}")
    return bumped


# ---------------------------------------------------------------------------
# Lookup (hot read path for the agent)
# ---------------------------------------------------------------------------
def lookup_entries(
    category: str,
    scope_key: str,
    ...
    try:
        if category not in ALLOWED_CATEGORIES:
            logger.warning(f"npv_memory_service - lookup: category '{category}' not allowed")
            return out
        slot = slot_name or get_slot_name()
        table_client = _get_table_client(slot, logger)
        if table_client is None:
        ...
        logger.error(f"npv_memory_service - lookup error: {e}")
    return out


# ---------------------------------------------------------------------------
# Orchestrator-driven write entrypoint: apply MemoryProposal items emitted by the agent
# ---------------------------------------------------------------------------
def apply_proposals(
...
# ---------------------------------------------------------------------------
# Admin operations (promote / deprecate / reactivate / manual upsert)
# ---------------------------------------------------------------------------
def set_status(ref: str, new_status: str, notes: Optional[str] = None,
    ...
    try:
        if new_status not in ALLOWED_STATUSES:
            logger.warning(f"npv_memory_service - set_status: invalid status '{new_status}'")
            return None
        pk, rk = parse_ref(ref)
        slot = slot_name or get_slot_name()
        table_client = _get_table_client(slot, logger)
        ...
        logger.error(f"npv_memory_service - set_status error: {e}")
        return None
...
def pin_manual(
    ...
    try:
        ...
        if ent is None:
            return None
        # Ensure manual rows are pinned High and active.
        ref = build_ref(ent["PartitionKey"], ent["RowKey"])
        return set_status(ref, "active", notes=notes, slot_name=slot_name, logger=logger)
    except Exception as e:
        logger.error(f"npv_memory_service - pin_manual error: {e}")
        return None


# ---------------------------------------------------------------------------
# Ad-hoc query (mirrors npv_archive_service.build_and_execute_adhoc_query style)
# ---------------------------------------------------------------------------
...
def build_and_execute_adhoc_query(query_fields: dict, slot_name: str, logger=logging) -> list:
    ...
    try:
        ...
        logger.info(f"npv_memory_service - adhoc query filter: {filter_string}")
        ...
        logger.error(f"npv_memory_service - build_and_execute_adhoc_query error: {e}")
    return results
...
# ---------------------------------------------------------------------------
# Cross-slot copy (used by the manual stage -> production promotion script)
# ---------------------------------------------------------------------------
        ...
        for ref in refs:
            try:
                ...
                if src is None:
                    ...
                    continue
                clone = dict(src)
                # Reset operational state for the target slot.
